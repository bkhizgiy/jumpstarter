...
get_latest_version() {
    ...
    local output
    if ! output="$(pip index versions --index-url "${index_url}" --pre jumpstarter-all 2>/dev/null)"; then
...
install_jumpstarter() {
    ...
    print_info "Installing jumpstarter-all==${version}..."
    if ! pip install --extra-index-url "${index_url}" "jumpstarter-all==${version}"; then
